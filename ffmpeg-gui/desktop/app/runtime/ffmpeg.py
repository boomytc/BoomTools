...
def build_command(
    *,
    ...
    args.extend(_operation_args(op, normalized, asset_path))
...
def _operation_args(operation: Operation, options: dict[str, Any], asset_path: Path | None) -> list[str]:
    ...
    if operation is Operation.subtitles:
        if not asset_path or not asset_path.exists():
            raise CommandError("subtitle file was not found")
        if asset_path.suffix.lower() not in SUBTITLE_FILE_EXTENSIONS:
            raise CommandError("subtitle file must be one of: .ass, .srt, .ssa, .vtt")
