...
class StackPanel(QGroupBox):
    ...
    def __init__(self) -> None:
        ...
        self.mode_label = QLabel("说明：仅支持可链式单输入滤镜，顺序即执行顺序。")
        layout.addWidget(self.mode_label)

        self.list_label = QLabel("Stack 队列：0项")
        ...
        layout.addWidget(QLabel("说明：切换操作后可继续补充栈项，保存顺序即执行顺序。"))
    ...
    def set_items(self, items: list[str]) -> None:
        ...
        self.list_label.setText(f"Stack 队列：{len(items)}项")
    ...
    def set_supported_note(self, supported: bool) -> None:
        if supported:
            self.mode_label.setText("说明：当前操作支持加入 Stack。")
            return
        self.mode_label.setText("说明：当前操作不支持加入 Stack。")
