...
- 长任务防睡眠：执行 ffmpeg 或打包 ZIP 时可临时阻止系统睡眠，macOS 使用 `caffeinate -dimsu`，Windows 使用 `SetThreadExecutionState`，Linux 使用 `systemd-inhibit`
...
设置中默认开启“长任务期间防止系统睡眠”。应用只会在 ffmpeg 任务或 ZIP 打包运行期间启用该保护，任务成功、失败、取消或关闭窗口后会停止。macOS 使用 `caffeinate -dimsu`；Windows 使用 `SetThreadExecutionState`；Linux 使用 `systemd-inhibit` 持有防睡眠锁。平台能力或命令不可用时只在状态栏提示，任务继续执行。
