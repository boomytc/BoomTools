...
- UI 设计服务于高频本机媒体处理工作流，整体风格应安静、清晰、紧凑，优先保证路径选择、参数确认、任务进度、日志排查和结果入口的可扫描性。
- 先建立设计系统基础，再做局部视觉细节：字体、间距、圆角、边框、背景、主色、状态色、弱化文本色、表格密度和日志可读性必须统一。
- 自定义 UI 优先沉淀为可复用组件、panel、delegate 或 QSS selector；不要把一次性控件、样式和状态逻辑堆回 `main_window.py`。
- 文件路径选择、状态徽标、进度展示、日志区域、命令预览、结果操作、任务表渲染等高复用元素必须组件化。
- 表格状态和进度使用 Qt model/view delegate 渲染，不在业务模型里混入绘制细节，不用表格内嵌大量子 widget。
- QSS 是视觉系统的集中入口，按基础控件、布局容器、按钮、输入、任务表、状态属性分区维护；使用 `objectName` 和 dynamic property 表达状态。
- 交互状态必须可感知：empty、invalid、busy、success、failure、disabled 都要有对应的视觉反馈或文案反馈。
