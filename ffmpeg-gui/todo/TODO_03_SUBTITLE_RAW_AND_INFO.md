...
- [ ] `subtitles` 参数增加：
  - [ ] `mode`: `soft|burn`
  - [ ] `output_format`: soft 支持 `mp4|mkv`，burn 支持 `mp4|webm|mov|mkv`
  - [ ] `font_size`: `small|medium|large`，仅 burn 使用
- [ ] soft 模式保持当前行为：
  - [ ] MP4 使用 `mov_text`。
  - [ ] MKV 复制字幕流。
- [ ] burn 模式：
  - [ ] 使用本机 ffmpeg `subtitles` filter。
  - [ ] 如果 ffmpeg 缺少相关 filter 或 libass 支持，显示明确错误。
  - [ ] 不实现浏览器 canvas 字幕渲染 fallback。
- [ ] 继续只允许 `.srt/.vtt/.ass/.ssa`。

## Raw Checklist

- [ ] Raw 参数仍使用 `shlex.split` 解析成参数数组。
- [ ] Raw 禁止用户自带主输入路径、主输出路径、`-i` 主输入替换、`-progress`、`pipe:`、`file:`。
- [ ] 新增可选第二输入：
  - [ ] 用户通过 GUI 选择文件。
  - [ ] runtime 将该路径作为受管 extra input。
  - [ ] Raw 参数可引用受管第二输入，但不能引用任意文件路径。
- [ ] 新增示例命令库：
  - [ ] drawbox watermark
  - [ ] cap framerate
  - [ ] grayscale
  ...
  - [ ] extract wav
  - [ ] first frame
  - [ ] replace audio with second input
- [ ] 示例命令只填充文本，不自动执行。

## Media Info Checklist

- [ ] 将当前文件加载时的 `ffprobe` 信息保留为快速信息。
- [ ] 增加深度扫描动作：
  - [ ] 执行 `ffmpeg -hide_banner -i input`。
  - [ ] 捕获 stderr 到日志区域。
  - [ ] 不生成输出文件。
- [ ] `media_info` 不进入批处理。
